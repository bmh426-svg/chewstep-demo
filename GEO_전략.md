...
> 버전 1.1 · 2026-07-26 작성 · 2026-07-28 수정
> v1.1 변경: §3에 '밥태기' 키워드 추가(메인 히어로·메타 병기 반영, 검색량 실측 과제 명시)
...
- **밥태기** (아이가 밥을 잘 안 먹는 시기 — 보호자 구어체) 〔2026-07-28 추가〕
  - 메인 히어로 문장(`밥태기도 이유가 있습니다`)·title·description·WebSite JSON-LD에 반영.
  - 검색어(`아이가 밥 안 먹는 이유`)와 **함께** 담는다 — 한 페이지가 여러 질의로 색인되는 것이 정상.
  - ⚠ 실측 필요: 검색량은 네이버 검색광고 키워드도구·구글 키워드플래너로 확인할 것.
    〔추정〕 맘카페·블로그 구어체라 네이버 비중이 클 가능성.
  - ⚠ 의학 용어가 아니다. 기간·원인을 단정하는 문장은 쓰지 않는다(진단 금지 원칙).
- 아이가 밥을 안 먹는 이유
- 이유식·유아식 거부 / 식감 거부
- 아기 식사 영상 분석 · 씹기 분석
