...
|---|---|---|
| `verify/consult-list-check.mjs` (화면 로직, Supabase 흉내) | **54/54 PASS** | — |
| `verify/consult-live-check.mjs` (**신설** · 실계정으로 배포본 접속) | — | **15/15 PASS** |

라이브(배포본)에서 확인한 사실 — `app.parent@chewstep.dev` 로 로그인:
`consultation_titles` 200 + 응답에 `view_count` 있고 `body`·`answer` 없음 · 목록 2건이 한 줄로
(제목 · 답변완료 · 👁 0 · `2026.08.06 11:51`) · 상세 진입 시 `consultation_view` 200 이 **1** 반환
(= 열람 1인 기록됨) · 콘솔 오류 0.
...
### 미확인 (남은 과제)
- 과거 열람 기록은 없다. 기존 글 2건의 조회수는 **배포 시점부터** 세기 시작한다(검증으로 1건 기록됨).
